...
pub fn parse_clipboard(nodes: &mut Vec<Node>, attr_style: AttrStyle, meta: Meta) -> Result<()> {
    let metas = meta
        .require_list()?
    ...
    for meta in metas {
        if meta.path().is_ident("copy") {
            parse_clipboard_copy(nodes, attr_style, meta)?;
        } else if meta.path().is_ident("paste") {
            parse_clipboard_paste(nodes, attr_style, meta)?;
        } else {
